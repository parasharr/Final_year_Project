<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ecommerce website</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://fontawesome.com/releases/v6.04.0/css/all.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
</head>
<body>
    <section id="header">
        <a href="home.html"><img src="logo.png" alt="logo" height="30px"></a>

        <div>
            <ul id="navbar">
                <li><a href="home.html">Home</a></li>
                <li><a class="active" href="shop.html">Shop</a></li>
                <li><a href="blog.html">Blog</a></li>
                <li><a href="about.html">About</a></li>
                <li><a href="contact.html">Contact Us</a></li>
                <li id="lg-bag"><a href="cart.html"><i class='bx bx-shopping-bag bag' style='color:#000000; font-size: 20px;' ></i></a></li>
                <li class="user"><a href="register.php"><i class="fa-solid fa-circle-user"></i></a></li>
                <a href="#" id="close"><i class="fa-solid fa-xmark" style="color: #000000; font-size: 25px;"></i></a>
            </ul>
        </div>
        <div id="mobile">
            <a href="cart.html"><i class='bx bx-shopping-bag' style='color:#000000' ></i></a>
            <i id="bar" class="fas fa-outdent"></i>
        </div>
    </section>

    <section id="page-header">
        <h2>#stayhome</h2>

        <p>save more by using coupons & up to 70% off!</p><br>
    </section>


    <section id="product1" class="section-p1">
        <div class="pro-container">
            <div class="pro" onclick="window.location.href='s-product.html'">
                <img src="y-dress-9.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Printed Black T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$78</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro" onclick="window.location.href='s-product2.html'">
                <img src="y-dress-10.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Printed Navy-Blue T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$81</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro" onclick="window.location.href='s-product3.html'">
                <img src="y-dress-11.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>White Printed T-shirt</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$77</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-12.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Black Tshirt</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$67</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-13.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Black printed T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$104</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-14.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Navy-Blue Printed T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$89</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-17.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Gray Printed T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$92</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-27.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Aesthetic Printed T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$56</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>

            <div class="pro">
                <img src="y-dress-25.jpeg" alt="">
                <div class="description">
                    <span>Lymio</span>
                    <h5>Casual Shirt for Men|| Shirt for Men|| Men Stylish Shirt</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$70</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-26.jpeg" alt="">
                <div class="description">
                    <span>Fitoda Fashion</span>
                    <h5>Men's Printed Lycra Half Sleeve V-Neck Lightweight and Comfortable</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$81</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-28.jpeg" alt="">
                <div class="description">
                    <span>LEWEL</span>
                    <h5>Men's Stylish Checkered Half Sleeve Shirt (Black)</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$77</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-29.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Men's Stylish Checkered Half Sleeve Shirt (Gray)</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$67</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-30.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Cartoon Astronaut Half-sleeve Shirts(Black)</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$104</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-31.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Classic Astronaut Shirts(Blue)</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$89</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-32.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Cartoon Astronaut Full Slave T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$92</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
            <div class="pro">
                <img src="y-dress-24.jpeg" alt="">
                <div class="description">
                    <span>adidas</span>
                    <h5>Cartoon Astronaut T-Shirts</h5>
                    <div class="star">
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                        <i class='bx bxs-star' style='color:#b3b318' ></i>
                    </div>
                    <h4>$56</h4>
                </div>
                <div>
                    <a href="#"><i class="fa-solid fa-cart-shopping cart"></i></a>
                </div>
            </div>
        </div>
    </section>

    <section id="pagination" class="section-p1">
        <a href="shop.html">1</a>
        <a href="shop2.html">2</a>
        <a href="#">3</a>
    </section>
        
    <footer class="section-p1">
        <div class="col">
            <img class="logo" src="logo.png" alt="logo"><br>
            <h4>Contact</h4>
            <p><b>Address:</b> 562 Wellington Road, Street 32, San Francisco</p>
            <p><b>Phone:</b>+01 2222 365/(+91)60035 16672</p>
            <p><b>Hours:</b>10:00 - 18:00, Mon - Sat</p>
            <div class="follow">
                <h4>Follow Us</h4>
                <div class="icon">
                    <a href="#"><i class="fab fa-facebook-f"></i></a>
                    <a href="#"><i class="fab fa-instagram"></i></a>
                    <a href="#"><i class="fab fa-youtube"></i></a>
                    <a herf="#"><i class="fab fa-pinterest-p"></i></a>
                    <a herf="#"><i class="fab fa-twitter"></i></a>
                </div>
            </div>
        </div>
        <div class="col">
            <h4>About</h4>
            <a href="#">About Us</a>
            <a href="#">Delivery Information</a>
            <a href="#">Privacy Policy</a>
            <a href="#">Terms & Conditions</a>
            <a href="#">Contact Us</a>
        </div>
        <div class="col">
        <h4>My Account</h4>
        <a href="#">Sign In</a>
        <a href="#">View Cart</a>
        <a href="#">My Wishlist</a>
        <a href="#">Track My Order</a>
        <a href="#">Help</a>
        </div>
        <div class="col-install">
            <h4>Install App</h4>
            <p>From App Store or Google PlayStore</p>
            <div class="row">
                <img src="app.jpg">
                <img src="play.jpg">
            </div>
            <p>Secure Payment Gateways</p>
            <img src="pay.png">
        </div>
        <div class="copyright">
            <p>2024, Pranjeet etc - HTML CSS Ecommerce Tamplate</p>
        </div>
        
    </footer>
    </section>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js" integrity="sha384-I7E8VVD/ismYTF4hNIPjVp/Zjvgyol6VFvRkX/vR+Vc4jQkC+hVqc2pM8ODewa9r" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.min.js" integrity="sha384-BBtl+eGJRgqQAUMxJ7pMwbEyER4l1g+O15P+16Ep7Q9Q+zqX6gSbd85u4mG4QzX+" crossorigin="anonymous"></script>
    <script src="script.js"></script>
</body>
</html>